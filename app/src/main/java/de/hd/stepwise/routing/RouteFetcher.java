...
public class RouteFetcher {
    ...
    public static List<GeoPoint> decodeGeoJson(String filepath) {
        List<GeoPoint> route = new ArrayList<>();
        try {
            String geoJson = new String(java.nio.file.Files.readAllBytes(java.nio.file.Paths.get(filepath)));
            JSONArray coords = new JSONArray(geoJson);
